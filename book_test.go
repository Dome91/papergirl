...
func TestUpdateComic(t *testing.T) {
	initializeTest()
	page1 := BookPage("B.jpg")
	page2 := BookPage("A.jpg")
	pages := []BookPage{page1, page2}
	comicBookContentStreamer = NewMockComicBookContentStreamer(pages)

	path := Path("path/book.cbz")
	bookStorage.Store(path, bytes.NewReader([]byte{1, 2, 3, 4}))
	existingBook := NewBook(path, CBZ, pages)
	books.Save(existingBook)
